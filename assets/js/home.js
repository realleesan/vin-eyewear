...
(function () {
    ...
    /* ============================================================
       0. HERO — BĂNG VIDEO KÉO ĐƯỢC BẰNG CHUỘT

       Markup: _layout/home/hero.php · Hình: components/video-hero.css

       ĐÃ THAY HẲN KHỐI "BA CLIP MỜ CHỒNG" (10/09/2026). Bản trước xếp ba thẻ
       <video> chồng khít và đổi opacity; nay ba tấm nằm cạnh nhau trên một
       băng và ta dời băng bằng translate3d.

       Khối này lo bốn việc, không hơn:
         1. kéo/vuốt để sang tấm khác, có kháng lực ở hai đầu băng
         2. ba vạch tiến độ bấm được, và phím mũi tên
         3. tự sang tấm sau sau AUTO_MS, dừng khi người dùng đang kéo
         4. dừng toàn bộ video khi hero cuộn khuất

       CẢ BA CLIP CÙNG PHÁT — quyết định có cân nhắc của chủ dự án, xem khối
       chú thích đầu _layout/home/hero.php để biết cái giá và cách quay về
       phương án một-clip nếu cần.

       TẮT JAVASCRIPT: băng không có transform nào từ máy chủ và .vhero__viewport
       thì overflow:hidden, nên tấm đầu chiếm trọn khung như một hero tĩnh; hai
       nút của nó là thẻ <a> thật. Ba vạch tiến độ bị CSS ẩn đi (html:not(.js))
       nên không có nút chết nào.
       ============================================================ */
    ...
    (function videoHero() {
        ...
        var track  = root.querySelector('[data-vhero-track]');
        var slides = Array.prototype.slice.call(root.querySelectorAll('.vhero__slide'));
        var clips  = Array.prototype.slice.call(root.querySelectorAll('.vhero__clip'));
        var bars   = Array.prototype.slice.call(root.querySelectorAll('[data-vhero-go]'));

        /* Dưới hai tấm thì không có gì để trượt — để nguyên hero tĩnh, và
           không gắn một trình nghe nào. */
        ...
        /* Đo bề ngang từ DOM chứ không từ window.innerWidth: hero có thể không
           chiếm trọn bề ngang cửa sổ (thanh cuộn, hoặc một khung bọc sau này),
           và lệch vài pixel là băng dừng không khớp mép. */
        function beNgang() {
        ...
        function render(px) {
            track.style.transform =
                'translate3d(' + (-at * beNgang() + (px || 0)) + 'px, 0, 0)';
        }

        /* inert trên tấm không hiện: nếu không, Tab sẽ chui vào hai nút của tấm
           đang nằm NGOÀI MÉP MÀN HÌNH — tiêu điểm biến mất và trình duyệt cuộn
           ngang theo nó. Đây là lỗi tiếp cận kinh điển của mọi băng trượt. */
        function danhDau() {
        ...
        function henAuto() {
            ...
            /* Không tự chạy khi hero đang khuất (không ai thấy) hoặc khi máy
               đặt giảm chuyển động — một tấm tràn bề ngang tự trượt là đúng
               loại chuyển động lớn mà thiết lập ấy muốn tránh. Video vẫn phát;
               chỉ việc TỰ SANG TẤM là dừng. */
            if (!nhinThay || prefersReducedMotion()) return;

            autoTimer = window.setTimeout(function () { di(at + 1); }, AUTO_MS);
        }
        ...
        /* ── KÉO ─────────────────────────────────────────────────────────────
           Pointer Events lo cả chuột, cảm ứng và bút bằng MỘT đường mã.
           setPointerCapture để cú kéo không đứt khi con trỏ ra khỏi băng —
           không có nó thì kéo nhanh ra ngoài mép là băng kẹt giữa chừng. */
        var batDau = 0;
        var dx     = 0;
        var dangKeo = false;
        var daDi    = false;
        ...
        track.addEventListener('pointerdown', function (e) {
            ...
            /* ┌─ KHÔNG BẮT CON TRỎ Ở ĐÂY — ĐÂY LÀ LỖI ĐÃ LÀM HAI NÚT CHẾT ──
               │ Bản trước gọi setPointerCapture ngay tại pointerdown. Hệ quả:
               │ MỌI sự kiện con trỏ sau đó đổi đích về .vhero__track, kể cả
               │ pointerup. Mà trình duyệt bắn `click` vào TỔ TIÊN CHUNG của
               │ đích pointerdown và đích pointerup — capture làm hai đích ấy
               │ đều thành track, nên click rơi vào track chứ không vào thẻ <a>
               │ đang nằm dưới ngón tay. Hai nút "Mua ngay" và "Khám phá bộ sưu
               │ tập" bấm không ăn, mà console không báo gì cả.
               │
               │ Nay chỉ bắt con trỏ KHI ĐÃ THẬT SỰ KÉO (xem pointermove). Cú
               │ bấm đứng yên không bao giờ đi qua capture, nên click về đúng
               │ thẻ <a> của nó.
               └──────────────────────────────────────────────────────────── */

            /* Gỡ .is-gliding: đang kéo thì băng phải bám tay TỪNG KHUNG HÌNH,
               còn transition thì làm nó chạy sau tay một nhịp. */
        ...
        track.addEventListener('pointermove', function (e) {
            ...
            /* Vượt 4px mới tính là KÉO. Đúng lúc ấy mới bắt con trỏ (để cú kéo
               không đứt khi tay ra khỏi băng) và mới đổi con trỏ sang "đang
               nắm". Dưới ngưỡng này thì mọi thứ vẫn là một cú bấm bình thường. */
            if (!daDi && Math.abs(dx) > 4) {
                daDi = true;
                track.classList.add('is-dragging');
                try { track.setPointerCapture(pid); } catch (err) { /* trình duyệt cũ */ }
            }

            if (!daDi) return;

            /* KHÁNG LỰC Ở HAI ĐẦU: kéo quá tấm đầu hoặc tấm cuối thì băng chỉ
               nhúc nhích 35% quãng tay. Băng cứng ngắc đọc ra như hỏng; băng đi
               hết cỡ rồi bật lại thì người dùng tưởng còn tấm nữa. */
            if ((at === 0 && dx > 0) || (at === tong - 1 && dx < 0)) dx *= 0.35;
        ...
        function ketThucKeo() {
            ...
            if (pid !== null) {
                try { track.releasePointerCapture(pid); } catch (err) { /* đã nhả */ }
                pid = null;
            }

            /* 12% bề ngang: đủ xa để không nhầm với một cú bấm hụt, đủ gần để
               không phải kéo hết màn hình. */
            var nguong = beNgang() * 0.12;

            if (dx <= -nguong)      di(at + 1);
            else if (dx >= nguong)  di(at - 1);
            else { render(0); henAuto(); }

            dx = 0;
        }

        track.addEventListener('pointerup', ketThucKeo);
        track.addEventListener('pointercancel', ketThucKeo);

        /* Kéo XONG thì cú click cuối cùng vẫn bắn ra, và nó rơi trúng thẻ <a>
           nằm dưới tay — người vừa trượt băng lại bị chuyển sang trang khác.
           Chặn ở pha CAPTURE để tới trước mọi trình nghe của chính thẻ <a>. */
        ...
        bars.forEach(function (b) {
            b.addEventListener('click', function () {
                di(parseInt(b.getAttribute('data-vhero-go'), 10) || 0);
            });
        });

        /* Phím mũi tên khi tiêu điểm đang ở trong hero. Không bắt ở document:
           mũi tên trái/phải là phím cuộn ngang của cả trang, cướp nó ở mọi nơi
           là chặn một đường đi lại hợp lệ. */
        root.addEventListener('keydown', function (e) {
            if (e.key === 'ArrowRight') { e.preventDefault(); di(at + 1); }
            else if (e.key === 'ArrowLeft') { e.preventDefault(); di(at - 1); }
        });

        /* Đổi bề ngang cửa sổ thì quãng dời tính theo bề ngang cũ sai hết. Vẽ
           lại KHÔNG có .is-gliding — đây là chỉnh lại vị trí, không phải một
           cú trượt, cho nó chạy animation là băng trôi vô cớ khi xoay máy. */
        window.addEventListener('resize', function () {
            track.classList.remove('is-gliding');
            render(0);
        }, { passive: true });

        /* Hero cuộn khuất -> dừng CẢ BA video. Ba luồng 1920px giải mã sau lưng
           người đang đọc phần dưới trang là CPU và pin đổ đi mà không ai thấy.
           Với phương án "cả ba cùng phát" thì đây là chỗ tiết kiệm duy nhất, nên
           nó càng đáng giữ.

           IntersectionObserver chứ không nghe 'scroll': trình duyệt tự tính,
           không tốn một lần đọc bố cục nào của ta. */
        if ('IntersectionObserver' in window) {
            new IntersectionObserver(function (entries) {
                entries.forEach(function (en) {
                    nhinThay = en.isIntersecting;

                    clips.forEach(function (c) {
                        if (en.isIntersecting) {
                            var p = c.play();
                            // Trình duyệt chặn autoplay: không có gì để cứu, và
                            // ném một lỗi chưa bắt ra console không giúp được ai.
                    ...
                    if (en.isIntersecting) henAuto(); else dungAuto();
                });
